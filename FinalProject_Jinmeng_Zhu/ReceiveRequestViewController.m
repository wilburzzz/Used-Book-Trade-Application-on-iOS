...
//#warning Incomplete method implementation.
    ...
    return [self.logInUser.receiveList.receiveList count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[[self.logInUser.receiveList.receiveList objectAtIndex:indexPath.row] forBook] bookName];
    cell.detailTextLabel.text = [[self.logInUser.receiveList.receiveList objectAtIndex:indexPath.row] watch];
    return cell;
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([[segue identifier] isEqualToString:@"viewreceive"]){
        UIViewController *dvc = segue.destinationViewController;
        ((ViewReceiveController *)dvc).request = self.selected;
        ((ViewReceiveController *)dvc).users = self.users;
        ((ViewReceiveController *)dvc).logInUser = self.logInUser;
    }
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    self.selected = (Request*)[self.logInUser.receiveList.receiveList objectAtIndex:indexPath.row];
    [self performSegueWithIdentifier:@"viewreceive" sender:self];
}
